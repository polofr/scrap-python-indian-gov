...
class Main:
    ...
    def process_input(browser, input):
        try:
            ...
            script = JsPreparator.get_list_of_panchayat_report(
                Main.financial_year,
                Main.financial_year_plan,
                Main.state_name,
                district_code,
                block_panch_code,
            )
            ...
            for row in all_rows:
                Main.csv_writer.line_queue.put(
                    f'{Main.financial_year}, {Main.state_name}, {state_code}, {district_name}, {district_code}, {block_panch_name}, {block_panch_code}, {panch_name}, {panch_code}, {plan_id}, {", ".join(row)}'
                )
    ...
    def run(villages=set()):
        ...
        try:
            script = JsPreparator.prepare_state(
                Main.financial_year, Main.financial_year_plan, Main.state_name
            )
            script += JsPreparator.get_list_of_districts(
                Main.financial_year, Main.state_name
            )
            JsExecutor.execute(browser, script)
            state_code = JsPreparator.get_state_code(Main.state_name)
            district_block_panch_tuples = (
                JsExecutor.parse_response_for_list_of_districts(
                    browser, Main.financial_year, state_code
                )
            )
        except Exception as ex:
            raise Exception(
                f"Failed to get list of districts for {Main.state_name} {Main.financial_year}: {str(ex)}"
            )
        ...
        for idx, district_block_panch_tuple in enumerate(district_block_panch_tuples):
            try:
                ...
                print(
                    f"Starting {idx} {block_panch_name} in district {district_name} in state {Main.state_name} for {Main.financial_year_plan}"
                )
                script = JsPreparator.prepare_state(
                    Main.financial_year, Main.financial_year_plan, Main.state_name
                )
                script += JsPreparator.get_list_of_panchayats(
                    Main.financial_year,
                    Main.state_name,
                    district_code,
                    block_panch_code,
                ...
                JsExecutor.execute(browser, script)
                panchs_tuples = JsExecutor.parse_response_for_list_of_panchs(
                    browser=browser
                )
                for panchs_tuple in panchs_tuples:
                    ...
                    plan_id = panchs_tuple[4]
                    unique_id = (
                        f"{panch_name.strip()}-{panch_code.strip()}-{plan_id.strip()}"
                    )
                ...
                print(
                    f"Failed to analyze {idx} {district_block_panch_tuple[0]} {district_block_panch_tuple[1]} : {str(ex)}"
                )
