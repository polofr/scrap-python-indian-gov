...
class JsExecutor:
    ...
    def parse_response_for_list_of_districts(browser, financial_year, state_code):
        result = WebDriverWait(browser, 10).until(
            ec.presence_of_element_located((By.ID, "statewise-report"))
        )
        str_res = result.get_attribute("innerHTML")
        district_block_panch_tuples_simple = re.findall(
            rf"onclick=\"javascript:getgpreport\('{state_code}',3,'{financial_year}','([0-9]+)','([0-9]+)',0\);\"",
            str_res,
        )
        ...
        if len(district_block_panch_tuples_simple) != len(district_block_panch_tuples):
            raise Exception(
                f"imperfect parsing in parse_response_for_list_of_districts: {len(district_block_panch_tuples_simple)} vs {len(district_block_panch_tuples)}"
            )
    ...
    def parse_response_for_list_of_panchs_2(browser):
        ...
        str_res = str_res.replace("\n", "")
        str_res_bis = re.search(
            r"SECTION 2 : Sectoral View(.*)SECTION 3 : Scheme View",
            str_res,
            flags=re.DOTALL,
        )
        ...
        if len(all_rows) == 0 or (
            all_rows[-1][0] is not None and all_rows[-1][0] != "Total"
        ):
            raise Exception(
                f"Unable to read table: {len(all_rows)} or {all_rows[-1][0]}"
            )
        all_rows[-1][0] = "Total"
        return all_rows
