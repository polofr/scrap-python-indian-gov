...
getreport('{state_code}', 3, '{financial_year}', '{state_name}');
    ...
    @staticmethod
    def get_list_of_panchayats(financial_year, state_name, district_code, block_panch_code):
        state_name = state_name.upper()
        state_code = STATES_SIMPLIFIED.get(state_name)
        if not state_code:
            raise Exception(f'{state_name} is not a correct state name')
...
getgpreport('{state_code}',3,'{financial_year}','{district_code}','{block_panch_code}',0);
    ...
    @staticmethod
    def get_list_of_panchayat_report(financial_year, financial_year_plan, state_name, district_code, block_panch_code):
        state_code = JsPreparator.get_state_code(state_name)
        script = JsPreparator.prepare_state(financial_year=financial_year, financial_year_plan=financial_year_plan,
