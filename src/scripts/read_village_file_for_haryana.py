    ...
    with open(file_path, "r") as original:
        ...
        for line in lines:
            ...
            all_villages[district][block_name].append(
                {"name": panchayat_name, "id": panchayat_id, "line": line}
            )
    ...
    with open(file_path, "r") as original:
        ...
        for line in lines:
            ...
            block_name = (
                ...
                .replace(" 1", "-I")
                .replace(" 2", "-II")
                .replace("Bhattu", "Bhattu Kalan")
                .replace("Ballabhgarh", "Ballabgarh")
                .replace("Nissing", "Nissing At Chirao")
                .replace("Meham", "Maham")
                .replace("Lakhan", "Lakhan Majra")
                .replace("GHARAUNDA (PART)", "GHARAUNDA")
                .replace("Block Saha", "Saha")
                .replace("Block Naraingarh", "Naraingarh")
                .replace("Block Shahzadpur", "Shahzadpur")
                .replace("Block Barara", "Barara")
                .upper()
                .strip()
                .replace("BLOCK ", f"{district_name}-")
            )
            ...
            if cmp_results[0]["score"] > 10:
                ...
                if selected_row is None:
                    for idx, cmp_result in enumerate(cmp_results[0:4]):
                        print(
                            "{:>2} {}".format(cmp_result["score"], cmp_result["match"])
                        )
                    print()
