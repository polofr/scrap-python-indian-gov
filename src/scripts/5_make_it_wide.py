...
def main():
    ...
    short_categories = [
        "AdministrativeTechnicalSupport",
        "AdultEducation",
        "Agriculture",
        "AnimalHusbandry",
        "CulturalActivities",
    ...
    for f in listdir(path):
        ...
        with open(file_path, "r") as original:
            with open(new_file_path, "w", newline="") as modified:
                modified.write(
                    "year, state, state_id, district_panchayat, district_panchayat_id, block_panchayat, block_panchayat_id, village_panchayat, village_panchayat_id, village_panchayat_budget_id, village_panchayat_budget_count"
                )
                for cat in short_categories:
                    modified.write(
                        f", sctied_{cat}, sttied_{cat}, generaltied_{cat}, totaltied_{cat}, scuntied_{cat}, stuntied_{cat}, generaluntied_{cat}, totaluntied_{cat}"
                    )
                modified.write("\n")
                csv_writer = csv.writer(
                    modified, delimiter=",", quotechar="|", quoting=csv.QUOTE_MINIMAL
                )
