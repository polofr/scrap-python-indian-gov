...
def set_line_length(lines_length, survey_position, village_name, line, origin):
    ...
    if entry is None:
        lines_length[survey_position] = {"lengths": [], "origins": []}
...
def main(argv):
    ...
    for survey_position, file_prefix in enumerate(file_prefixes):
        for file_suffix in file_suffixes:
            file_path = (
                ...
                / "scripts_final_merge"
                / "csv_files_corrected"
                / f"{file_prefix}_Survey_{file_suffix}.csv"
            )
            ...
            try:
                ...
                raise Exception(
                    f"Failed for {file_prefix}_Survey_{file_suffix}.csv : {str(exp)}"
                )
    ...
    for village_id, village in sorted(village_set.items()):
        if (
            len(village["names"][0])
            and len(village["names"][1])
            and len(village["names"][2])
            and len(village["names"][3])
            and len(village["names"][4]) > 3
        ):
            complete_villages += 1
    ...
    for idx, line_length in enumerate(lines_length):
        print(
            f"For {file_prefixes[idx]}, not all surveys have the same number of columns"
        )
