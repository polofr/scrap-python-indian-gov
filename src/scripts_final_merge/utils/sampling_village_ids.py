...
class SamplingVillageIds:
        ...
        with open(file_path, "r", encoding="utf-8") as original:
            ...
            for line in lines:
                ...
                if gender_reservation == "F":
                    ...
                    raise Exception(
                        f"gender_reservation is {gender_reservation} for {line}"
                    )
                if caste_reservation == "OBC":
                    ...
                    raise Exception(
                        f"caste_reservation is {caste_reservation} for {line}"
                    )
        ...
        with open(file_path, "r", encoding="utf-8") as original:
            ...
            for line in lines:
                ...
                if caste_reservation == "OBC":
                    ...
                    caste_reservation == "General"
                    or caste_reservation == "GENRAL"
                    or caste_reservation == "GENERAL"
                ):
                    caste = "O"
                else:
                    raise Exception(
        ...
        with open(file_path, "r", encoding="utf-8") as original:
            ...
            for line in lines:
                if skip_first is True:
                    skip_first = False
                    continue
                from src.scripts_dta.find_specific_villages_ahmednagar import (
                    set_reservation_for_ahmednagar,
                )

                set_reservation_for_ahmednagar(
                    SamplingVillageIds.village_id_to_reservation,
                    line[0],
                    line[1],
                    line[2],
                )
        ...
        with open(file_path, "r", encoding="utf-8") as original:
            ...
            for line in lines:
                if skip_first is True:
                    skip_first = False
                    continue
                from src.scripts_dta.find_specific_villages_pune import (
                    set_reservation_for_pune,
                )

                set_reservation_for_pune(
                    SamplingVillageIds.village_id_to_reservation, line[1], line[0]
                )
        ...
        with open(file_path, "r") as original:
            ...
            for line in lines:
                ...
                SamplingVillageIds.all_villages["3"].append(
                    {"name": panchayat_name, "id": panchayat_id}
                )
    ...
    def find_best_match(village_name, district):
        if district.lower() == "solapur":
            district = "1"
        village_name = village_name.replace("Grampanchayat", "").replace(
            "grampanchayat", ""
        )
        cmp_results = []
        for village in SamplingVillageIds.all_villages[district]:
            cmp_results.append(
                {
                    "score": textdistance.damerau_levenshtein(
