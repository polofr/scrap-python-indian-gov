...
def main(argv):
    ...
    for file_suffix in file_suffixes:
        file_path = (
            PROJECT_ROOT
            / f"src/scripts_final_merge/csv_files/Upa_Sarpanch_Survey_{file_suffix}.csv"
        )
        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8") as original:
                ...
                for idx, line in enumerate(lines):
                    ...
                    if result is None:
                        print(
                            f"Could not find a village id at line {idx + 1} in Upa_Sarpanch_Survey_{file_suffix}.csv for {instanceid} {district} {villagename}"
                        )
                        ...
                        if result != expected_result:
                            print(
                                f"Found a village id for {instanceid} in Upa_Sarpanch_Survey_{file_suffix}.csv but {json.dumps(result)} vs {json.dumps(expected_result)}"
                            )
            ...
            raise Exception(
                f"Failed for Upa_Sarpanch_Survey_{file_suffix}.csv : {str(exp)}"
            )
