                ...
                with open(file_path, "r", encoding="utf-8") as original:
                    ...
                    for line in lines:
                        if skip_first is True:
                            ...
                            print(
                                f"{district_column_pos}, {villageid_column_pos}, {villagename_column_pos} for {file_prefix}{file_suffix}.csv"
                            )
                        ...
                        if instanceid in instanceid_set:
                            print(
                                f"Duplicate {instanceid} inside {file_prefix}{file_suffix}.csv"
                            )
                        instanceid_set.add(instanceid)
            except Exception as exp:
                raise Exception(
                    f"Failed for {file_prefix}{file_suffix}.csv : {str(exp)}"
                )
