...
def set_reservation_for_ahmednagar(
    village_set, village_id, caste_reservation, gender_reservation
):
...
def main():
    ...
    with open(file_path, "r", encoding="utf-8") as original:
        ...
        for line in lines:
            if skip_first is True:
                skip_first = False
                continue
            set_reservation_for_ahmednagar(
                village_id_to_reservation, line[0], line[1], line[2]
            )
    ...
    with open(file_path, "r", encoding="utf-8") as original:
        ...
        print(
            f"Found {len(selected_villages)} in sarpanch survey for Ahmednagar district"
        )
    ...
    with open(file_path, "r", encoding="utf-8") as original:
        ...
        for line in lines:
            ...
            if line[22] == "3.0" or line[22] == "3":
                ...
                set_village_name(
                    village_id_to_names, line[16], "upa-sarpanch", line[21]
                )
        print(
            f"Found {len(selected_villages)} in Upa_Sarpanch survey for Ahmednagar district"
        )
    ...
    with open(file_path, "r", encoding="utf-8") as original:
        ...
        print(
            f"Found {len(selected_villages)} in Notable survey for Ahmednagar district"
        )
    ...
    with open(file_path, "r", encoding="utf-8") as original:
        ...
        print(
            f"Found {len(selected_villages)} in Gram_Sevak survey for Ahmednagar district"
        )
    ...
    for village_id, surveys in village_set.items():
        ...
        if surveys[0] != 1:
            print(
                f"Weird more than one survey for sarpanch {village_id} {village_id_to_names[village_id]}"
            )
            continue
        if surveys[1] == 0:
            print(
                f"Missing upa-sarpanch {village_id} {village_id_to_names[village_id]}"
            )
            continue
        if surveys[1] != 1:
            print(
                f"Weird more than one survey for upa-sarpanch {village_id} {village_id_to_names[village_id]}"
            )
            continue
        if surveys[2] == 0:
        ...
        if surveys[2] != 1:
            print(
                f"Weird more than one survey for gram-sevak {village_id} {village_id_to_names[village_id]}"
            )
            continue
        if surveys[3] == 0:
        ...
        if surveys[3] != 1:
            print(
                f"Weird more than one survey for group {village_id} {village_id_to_names[village_id]}"
            )
            continue
        if surveys[4] < 4:
            print(
                f"Missing only {surveys[4]} notable {village_id} {village_id_to_names[village_id]}"
            )
            continue
        villages_with_all.append(village_id)
        if village_id_to_reservation.get(village_id) is None:
            print(
                f"Missing reservation information {village_id} {village_id_to_names[village_id]}"
            )
            continue
        new_entry = [
            village_id,
            village_id_to_reservation[village_id][0],
            village_id_to_reservation[village_id][1],
            village_id_to_gan_sevac_sex[village_id],
            village_id_to_names[village_id][0],
        ]
        new_csv.append(new_entry)
    ...
    new_file_path = PROJECT_ROOT / "csv_files/results/result_ahmednagar.csv"
